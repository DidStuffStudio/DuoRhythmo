...
handlers.makeEntityAPICallGetObject = function (args, context) {
    ...
    // The pre-defined 'entity' object has functions corresponding to each PlayFab Entity API,
    // including 'SetObjects' (https://api.playfab.com/documentation/Data/method/SetObjects).
    var apiResult = entity.GetObjects({
        Entity: {
     		Id: args.friendId,
    ...
    log.info("These are the results: " + apiResult.ObjectResult);

    return {
        profile: args.friendId,
        getResult: apiResult.ObjectResult
        // setResult: apiResult.SetResults[0].SetResult
    };
...
handlers.updateMatchInvitation = function (args, context) {
    ...
    return {
        profile: args.ReceiverEntityId,
        setResult: apiResult.SetResults[0].SetResult
    };
};

// https://community.playfab.com/questions/35067/how-to-look-up-player-via-titledisplayname-via-clo.html
handlers.AdminGetUserAccountInfoPlayfabId = function (args, context) {
    let url = "https://[TitleId].playfabapi.com/Admin/GetUserAccountInfo"; // TODO --> replace [TitleId] with your titleId
    let method = "POST";
    let contentBody = JSON.stringify(args);
    let contentType = "application/json";
    let headers = { "X-SecretKey": "[SecretKey]" }; // TODO --> Replace with your secret key
    let responseString = http.request(url, method, contentBody, contentType, headers);
    let responseJSONObj = JSON.parse(responseString);
    log.info(responseJSONObj);
...
handlers.SendFriendRequest = function(args, context) {
    ...
    let friendPlayfabId = handlers.AdminGetUserAccountInfoPlayfabId( { Username:  args.FriendUsername} )["data"]["UserInfo"]["PlayFabId"];
    log.info("Friend Playfab Id response: " + friendPlayfabId);
    // log.info("friend playfab id: " + friendPlayfabId["Username"]);
    server.AddFriend({
...
handlers.DeletePlayerAccount = function(args, context) {
    server.DeletePlayer({
       PlayFabId: currentPlayerId, 
    });
}
